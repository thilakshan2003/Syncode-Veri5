...
export default function TestKitCard({ title, description, price, features = [], badge, imageSrc }) {
    return (
        <div className="bg-card rounded-3xl p-6 border border-border hover:border-primary shadow-lg hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300 flex flex-col h-full relative overflow-hidden group">
            ...
            <Button className="w-full bg-primary hover:bg-primary/90 text-white rounded-xl h-10 font-bold shadow-md shadow-primary/10 active:scale-[0.98] transition-all">
                Order Now
            </Button>
        </div>
    );
